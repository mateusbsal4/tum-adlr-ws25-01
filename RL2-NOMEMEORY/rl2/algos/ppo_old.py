...
def training_loop(
    ...
    for pol_iter in range(pol_iters_so_far, max_pol_iters):
        ...
        # for i in range(0, meta_episodes_per_policy_update): 
            ...
            local_timesteps += len(rewards[rewards != 0])
            total_timesteps += comm.allreduce(local_timesteps, op=MPI.SUM)
            # print("local: ", local_timesteps, " total timesteps: ", total_timesteps)
            
            if comm.Get_rank() == ROOT_RANK and show_pbar:
                    episode_pbar.update(total_timesteps)

            
            # flag any successful or ununsual episodes
            if last_reward != -100:
                print("Success after ", len(rewards[rewards != 0])," steps, reward: ", last_reward)
                logging.info("Success after ", len(rewards[rewards != 0])," steps, reward: ", last_reward)
        ...
        for opt_epoch in range(ppo_opt_epochs):
            ...
            idxs = np.random.permutation(episode_count)
            if comm.Get_rank() == ROOT_RANK:
                logging.info(f"pol update {pol_iter}, opt_epoch: {opt_epoch}...")
                print(f"\npol update {pol_iter}, opt_epoch: {opt_epoch}...\n")
                if show_pbar:
                    weight_pbar = tqdm(total=(episode_count//meta_episodes_per_learner_batch), desc="Updating Weights")
                
            # optional: progress bar
            # for i in range(0, meta_episodes_per_policy_update, meta_episodes_per_learner_batch):
            for i in range(0, episode_count, meta_episodes_per_learner_batch):
