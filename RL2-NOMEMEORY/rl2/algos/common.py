...
meta-reinforcement learning agents without memory.
"""
...
def generate_meta_episode(
    ...
    with decisions being made by an agent without state preserved
    ...
    while not done_t:
        ...
        if meta_episode.timestep == 1000-1 and r_t != 0:
            r_t = -100
            print("Terminated due to timeout.", r_t)
            time.sleep(1)
            
        if r_t == 100:
            print("Terminated due to success.", r_t)
            time.sleep(10)
